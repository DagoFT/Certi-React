...
function createCrystals(player, ai) {
    const crystals = [];
    while (crystals.length < 5) {
        const position = randomPosition();
        const occupiedByPlayer = player.x === position.x &&
            player.y === position.y;
        const occupiedByAI = ai.x === position.x &&
            ai.y === position.y;
        const alreadyExists = crystals.some((crystal) => crystal.x === position.x &&
...
function createGame() {
    ...
    return {
        ...
        player: playerData,
        ai: aiData,
        crystals: createCrystals(playerData, aiData),
...
function movePlayer(game, direction) {
    ...
    game.player.x = x;
    game.player.y = y;
    const crystalIndex = game.crystals.findIndex((crystal) => crystal.x === x &&
        crystal.y === y);
    if (crystalIndex !== -1) {
...
function moveAI(game, direction) {
    let x = game.ai.x;
    let y = game.ai.y;
    if (direction === "up")
        y--;
...
app.post("/api/games/:id/action", (request, response) => {
    ...
    const { action, direction } = request.body;
    let playerAction;
    if (action === "move") {
        if (direction !== "up" &&
            direction !== "down" &&
        ...
        const attacked = attack(game.player, game.ai);
        if (!attacked) {
            return response.status(400).json({
                mensaje: "El enemigo debe estar al lado para atacar",
            });
        }
        playerAction = {
            type: "attack",
    ...
    const aiAction = decideAIAction(game.ai, game.player, game.crystals);
...
app.listen(PORT, () => {
    console.log(`Neon Salvage ejecutándose en el puerto ${PORT}`);
});
